...
Choco the goat headbops to the beat of whatever's playing on your system audio. Drop the overlay into Streamlabs as a transparent browser source for a live reactive Twitch overlay.
...
- `overlay.html` connects, scales + tilts the goat on each beat, transparent background ready for OBS/Streamlabs
...
2. In **Streamlabs** → your scene → **Add Source** → **Browser Source**:
   - **URL** (Local file checkbox ON): `C:\Users\thijm\source\repos\Chocojam\overlay.html`
...
If the goat bops too often / not enough, open `beats.py` and tweak:
...
- **Goat bops on speech/silence** → lower `BEAT_THRESHOLD` or narrow the bass band.
